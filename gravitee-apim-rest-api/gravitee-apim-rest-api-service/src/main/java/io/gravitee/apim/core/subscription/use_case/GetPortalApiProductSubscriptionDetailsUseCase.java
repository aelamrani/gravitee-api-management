...
public class GetPortalApiProductSubscriptionDetailsUseCase {
    ...
    private List<ApiSummary> resolveApis(ApiProduct apiProduct, Input input) {
        ...
        return apiIds
            .stream()
            .map(apiId -> toApiSummary(apiId, apisById.get(apiId), navigationItemsByApiId.getOrDefault(apiId, List.of()), input))
            .sorted(
    ...
    private ApiSummary toApiSummary(String apiId, Api api, List<PortalNavigationApi> navigationItems, Input input) {
        if (api == null) {
        ...
        return new ApiSummary(
            ...
            availability,
            entrypoints,
            resolveDocumentationTarget(navigationItems, input)
        );
    }
    ...
    private DocumentationTarget resolveDocumentationTarget(List<PortalNavigationApi> navigationItems, Input input) {
        return navigationItems
            .stream()
            .filter(item -> isAccessibleFromSubscribedProduct(item, input))
    ...
    private boolean isAccessibleFromSubscribedProduct(PortalNavigationApi item, Input input) {
        ...
        while (current != null && visited.add(current.getId())) {
            if (input.viewerContext().shouldNotShow(current)) {
                return false;
            }
            if (
            ...
            current = portalNavigationItemsQueryService.findByIdAndEnvironmentId(input.environmentId(), current.getParentId());
        }
