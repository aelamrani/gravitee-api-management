...
public class PortalNavigationBulkImportDomainService {
    ...
    public BulkImportResult importSubtree(PortalNavigationFolder rootFolder) {
        ...
        try {
            ...
            source.setLastFetchError("Unable to list files from source type %s.".formatted(source.getSourceType()));
            crudService.update(rootFolder);
            return BulkImportResult.listingFailure(rootFolder.getTitle(), source.getLastFetchError());
        }
        ...
        if (files.isEmpty()) {
            ...
            source.setLastFetchError("The source of type %s listed no files.".formatted(source.getSourceType()));
            crudService.update(rootFolder);
            return BulkImportResult.listingFailure(rootFolder.getTitle(), source.getLastFetchError());
        }
