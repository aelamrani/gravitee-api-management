...
export class ApiProductSubscriptionApiAccessComponent {
  ...
  protected readonly curlCommand = computed(() => this.formatCurlCommandLine(this.selectedEntrypointValue()));

  private formatCurlCommandLine(entrypointUrl: string): string {
    if (!entrypointUrl) {
      return '';
    }

    let curlHeader = '';
    switch (this.planSecurity()) {
      case 'JWT':
      case 'OAUTH2':
        curlHeader = '--header "Authorization: Bearer {{ ACCESS_TOKEN }}" ';
        break;
      case 'API_KEY': {
        const apiKeyHeader = this.configService.configuration.portal?.apikeyHeader;
        if (apiKeyHeader) {
          curlHeader = `--header "${apiKeyHeader}: ${this.apiKey() || '{{ API_KEY }}'}" `;
        }
        break;
      }
    }
