...
/** Key the email field shares with the server's `email.invalid` attribution. */
...
export function SignUpPage() {
    ...
    const [customUserFieldsError, setCustomUserFieldsError] = useState('');
    ...
    useEffect(() => {
        ...
        fetchCustomUserFields()
            ...
            .catch((error: unknown) => {
                if (active) {
                    setCustomUserFieldsError(error instanceof Error ? error.message : 'The additional questions could not be read.');
                }
            })
            .finally(() => {
                if (active) {
    ...
    const emailError = fieldErrors[EMAIL_FIELD] ?? (emailTouched && email.length > 0 && !emailValid ? INVALID_EMAIL : '');
    ...
    /** A rejection describes the value that was sent, so editing that value retires it. */
    ...
    return (
        ...
        >
            <form onSubmit={handleSubmit} className="space-y-6" noValidate>
                {customUserFieldsError ? (
                    <Alert role="alert">
                        <AlertTitle>{"Couldn't load the additional questions"}</AlertTitle>
                        <AlertDescription>{customUserFieldsError}</AlertDescription>
                    </Alert>
                ) : null}
 ...
 */
function CustomUserFieldControl({
    field,
    value,
    onChange,
    error,
    ...
    return (
        <Field orientation="vertical" className="gap-2" data-invalid={error ? true : undefined}>
