...
describe('SignUpPage', () => {
    describe('the identity form', () => {
        ...
        it('renders a rejected email against the email field, not as a toast', async () => {
            ...
            const emailError = await screen.findByText('Value [ada@example.com] is not a valid email.');
            expect(screen.getByLabelText(/Email/).closest('[data-slot="field"]')?.contains(emailError)).toBe(true);
        });

        it('reports a rejection no field owns against the form', async () => {
            const user = userEvent.setup();
            server.use(
                http.post(REGISTRATION_URL, () =>
                    HttpResponse.json({ message: 'User registration is not available.', http_status: 400 }, { status: 400 }),
                ),
            );
            renderSignUpPage();
            ...
            expect(await screen.findByText('User registration is not available.')).toBeTruthy();
    ...
    describe('the configured user fields', () => {
        ...
        it('renders a rejected custom field against that field', async () => {
            const user = userEvent.setup();
            server.use(
                http.post(REGISTRATION_URL, () =>
                    HttpResponse.json(
                        {
                            message: "Unable to create the custom user field 'job_title'",
                            http_status: 400,
                            technicalCode: 'custom-user-field',
                            parameters: { key: 'job_title', action: 'create' },
                        },
                        { status: 400 },
                    ),
                ),
            ...
            const fieldError = await screen.findByText("Unable to create the custom user field 'job_title'");
            expect(
                screen
                    .getByLabelText(/Job title/)
