...
export type RegistrationResult =
    ...
    /** Refused over one named input -- `email`, or the key of a custom user field. */
    | { readonly outcome: 'field-rejected'; readonly field: string; readonly message: string }
    /** Refused for a reason no single input owns. */
...
/**
 * Technical codes this endpoint can answer with, and the input each belongs to.
 *
 * `user.invalid` is deliberately absent: see {@link isAccountExistenceOracle}.
 */
const EMAIL_REJECTED_CODE = 'email.invalid';
const CUSTOM_FIELD_REJECTED_CODE = 'custom-user-field';
...
function toResult(error: ApiError): RegistrationResult {
    ...
    if (error.technicalCode === EMAIL_REJECTED_CODE) {
        return { outcome: 'field-rejected', field: 'email', message: error.message };
    }
    const rejectedKey = error.technicalCode === CUSTOM_FIELD_REJECTED_CODE ? error.parameters?.key : undefined;
    if (rejectedKey) {
        return { outcome: 'field-rejected', field: rejectedKey, message: error.message };
    }
    return { outcome: 'rejected', message: error.message || UNEXPECTED_FAILURE };
}
...
export type ActivationResult =
    ...
    | { readonly outcome: 'password-rejected'; readonly message: string }
...
/** A live check of the organization's setting, not a verdict on the link. */
...
function toActivationResult(error: ApiError): ActivationResult {
    ...
    if (code === 'passwordFormat.invalid') {
        return { outcome: 'password-rejected', message: error.message };
    }
    ...
    // A server failure's message is the exception's own text: an expired or forged token arrives
    // here as the JWT library's wording, wrapped as a 500 `unexpected`. None of it is written for
    // the reader, so the page's own sentence stands in.
    if (error.status >= 500) {
        return { outcome: 'rejected', message: UNEXPECTED_ACTIVATION_FAILURE };
    }
    return { outcome: 'rejected', message: error.message || UNEXPECTED_ACTIVATION_FAILURE };
}
