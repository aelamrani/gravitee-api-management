...
describe('submitRegistration', () => {
    ...
    it('attributes a rejected email to the email field', async () => {
        rejectRegistration({ message: 'Value [ada] is not a valid email.', technicalCode: 'email.invalid' });

        await expect(submitRegistration({ ...IDENTITY, email: 'ada' })).resolves.toEqual({
            outcome: 'field-rejected',
            field: 'email',
            message: 'Value [ada] is not a valid email.',
        });
    });

    it('attributes a rejected custom user field to that field', async () => {
        rejectRegistration({
        ...
        await expect(submitRegistration({ ...IDENTITY, customFields: { 'Job Title': 'Analyst' } })).resolves.toEqual({
            outcome: 'field-rejected',
            field: 'Job Title',
            message: "Unable to  the custom user field 'Job Title'",
        });
    ...
    it('reports a rejection no single field owns against the form', async () => {
        rejectRegistration({ message: 'User registration is not available.', technicalCode: 'user.registration.unavailable' });
...
describe('finalizeRegistration', () => {
    ...
    it('attributes a refused password to the password field', async () => {
        ...
        await expect(finalizeRegistration(ACTIVATION)).resolves.toEqual({
            ...
            message: 'The password is not valid according to policy rules.',
        });
    });
    ...
    it('passes on the wording of a client error it cannot attribute', async () => {
        rejectFinalize({ message: 'Too many attempts. Wait a minute and try again.', technicalCode: 'errors.rate-limit' }, 429);
